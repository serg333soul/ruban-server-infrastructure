...
for TARGET_FOLDER in "${!RULES[@]}"; do
    ...
    # Знаходимо файли і переміщуємо їх, виводячи імена для логування
    while IFS= read -r file; do
        if mv "$file" "$FULL_TARGET_PATH/"; then
            ...
            log "ПОМИЛКА при переміщенні: $file"
        fi
    done < <(find "$INBOX" -maxdepth 1 -type f \( "${FIND_ARGS[@]}" \) -print)

    if [ $MOVED_COUNT -gt 0 ]; then
        log "Категорія $TARGET_FOLDER: оброблено $MOVED_COUNT файлів."
    fi
done
...
if [ $CHANGES_DETECTED -eq 1 ]; then
    ...
    if docker exec -u 33 "$CONTAINER_NAME" php occ files:scan --path="/$NC_USER/files"; then
        log "База даних успішно оновлена."
    else
        log "ПОМИЛКА при оновленні бази даних."
        exit 1
    fi
else
